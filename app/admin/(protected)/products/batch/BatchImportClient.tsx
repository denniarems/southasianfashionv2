...
} from '@/app/actions/admin/batch-import'
...
export default function BatchImportClient({ collections, categories, models }: BatchImportClientProps) {
	...
	const hydratePreviewRows = (rows: CsvRow[], folders: Record<string, FolderContent>) => {
		...
		setExpandedRows(
			Object.fromEntries(hydrated.map((row, idx) => [row.index, idx === 0])),
		)
	}
	...
	const startImport = () => {
		startImportTransition(() => {
			void (async () => {
				...
				setProgressLabel('Uploading reference files to Blob...')
	...
	return (
		<div className="space-y-6">
			<div className="flex items-center justify-between gap-4">
				<div>
					<h1 className="text-2xl font-heading text-stone-900 tracking-wide">Batch Product Import</h1>
					...
					<div className="border border-stone-200 bg-white">
						...
						>
							<div>
								...
								<p className="text-xs text-stone-500">CSV format, folder structure, desc.txt tips, and Sheets export</p>
							</div>
							...
							<div className="p-4 space-y-5 text-sm text-stone-700">
								<section className="space-y-2">
									...
									<p className="text-xs text-stone-500">
										All columns are required. Price must be numeric (no CAD symbol). `isNew` / `isFeatured`
...
Semi-stitched, fits waist 26-32 inches.`}</pre>
						...
						<label className="block border border-dashed border-stone-300 bg-white p-4 cursor-pointer hover:border-stone-500 transition-colors">
							<div className="flex items-start gap-3">
								...
								<div>
									...
									{folderFileCount > 0 && (
										<p className="text-xs text-emerald-700 mt-2">Indexed files: {folderFileCount}</p>
									)}
								</div>
							</div>
							<input
				...
				<div className="space-y-4">
					<div className="sticky top-4 z-10 bg-white border border-stone-200 px-4 py-3 flex flex-wrap gap-3 items-center justify-between">
						<div className="text-xs text-stone-600 flex flex-wrap gap-3">
							...
							<span>Images: <strong>{imageCount}</strong></span>
							<span>desc.txt: <strong>{descCount}</strong></span>
							<span className="text-emerald-700">✅ Ready: <strong>{rowHealth.ready.length}</strong></span>
							<span className="text-amber-700">⚠️ Warnings: <strong>{rowHealth.warning.length}</strong></span>
							<span className="text-red-700">❌ Errors: <strong>{rowHealth.error.length}</strong></span>
						</div>
						<Button
							type="button"
							variant="outline"
								...
								<div key={row.index} className="border border-stone-200 bg-white">
									...
									>
										<div className="flex items-center gap-3">
											<span className="text-xs border border-stone-300 px-2 py-0.5">#{row.index}</span>
											<div>
												<p className="text-sm font-medium text-stone-900">{row.name || '(Unnamed product)'}</p>
												<p className="text-xs text-stone-500">{row.category || 'No category'} · CAD {Number.isFinite(row.price) ? row.price : '-'}</p>
											</div>
										</div>
										<div className="flex items-center gap-2">
											...
											{expandedRows[row.index] ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
										</div>
									</button>
									...
									{expandedRows[row.index] && (
										<div className="px-4 pb-4 space-y-4 border-t border-stone-100">
											<div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3 pt-3">
												...
												<div>
													...
													<Input
														type="number"
														value={row.price}
														onChange={(e) => updatePreviewRow(row.index, { price: Number(e.target.value) })}
														className="rounded-none mt-1"
													/>
												</div>
												<div>
													...
													<Input
														value={row.category}
														onChange={(e) => updatePreviewRow(row.index, { category: e.target.value })}
														className="rounded-none mt-1"
													/>
												</div>
												<div>
													...
													<Input
														...
														onChange={(e) => updatePreviewRow(row.index, { collection: e.target.value })}
														className="rounded-none mt-1"
													/>
												</div>
												<div className="flex items-center justify-between border border-stone-200 px-3 py-2 mt-5">
													...
													<Switch
														checked={row.isNew}
														onCheckedChange={(checked) => updatePreviewRow(row.index, { isNew: checked })}
													/>
												</div>
												<div className="flex items-center justify-between border border-stone-200 px-3 py-2 mt-5">
													...
													<Switch
														...
														onCheckedChange={(checked) => updatePreviewRow(row.index, { isFeatured: checked })}
													/>
												</div>
											</div>
											...
											<div>
												...
												<Textarea value={row.descText} readOnly className="rounded-none min-h-24 bg-stone-50" />
											</div>
					...
					<div className="flex items-center justify-between">
						<Button type="button" variant="outline" onClick={() => setStep(1)} className="rounded-none text-xs">
							Back
						</Button>
						<Button
							type="button"
			...
			{step === 3 && (
				<div className="space-y-4 border border-stone-200 bg-white p-4">
					...
					{summary && (
						<div className="border border-stone-200 p-3 bg-stone-50 space-y-2">
							...
							<div className="grid gap-2 md:grid-cols-5 text-xs">
								<div>Created: <strong>{summary.created}</strong></div>
								<div>Skipped: <strong>{summary.skipped}</strong></div>
								<div>Descriptions: <strong>{summary.descriptions}</strong></div>
								<div>Generated Images: <strong>{summary.generatedImages}</strong></div>
								<div>Errors: <strong>{summary.errors.length}</strong></div>
							</div>
					...
					<div className="flex items-center justify-between">
						<Button type="button" variant="outline" onClick={() => setStep(2)} className="rounded-none text-xs">
							Back to Preview
						</Button>
						<Button
							type="button"
			...
			<Dialog open={previewModalOpen} onOpenChange={setPreviewModalOpen}>
				<DialogContent className="max-w-2xl rounded-none">
					...
					<DialogFooter>
						<Button type="button" onClick={() => setPreviewModalOpen(false)} className="rounded-none">
							Close
						</Button>
					</DialogFooter>
				</DialogContent>
			...
			<Dialog open={Boolean(lightboxImage)} onOpenChange={(open) => !open && setLightboxImage('')}>
				<DialogContent className="max-w-3xl rounded-none p-3">
					<div className="border border-stone-200 bg-stone-100">
						{lightboxImage ? (
							/* eslint-disable-next-line @next/next/no-img-element */
							<img src={lightboxImage} alt="Reference" className="w-full max-h-[70vh] object-contain" />
						) : (
