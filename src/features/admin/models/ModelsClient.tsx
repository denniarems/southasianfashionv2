...
export default function ModelsClient({ initialModels }: { initialModels: any[] }) {
	...
	const handleGenerate = async () => {
		...
		try {
			const res = await generateModelImage({
				data: { prompt: description, style, ageRange, gender, ethnicity },
			})
	...
	const handleDelete = (id: string) => {
		...
		startDeleting(async () => {
			const res = await deleteModel({ data: { id } })
		...
		<div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8">
			...
			<Tabs defaultValue="create" className="w-full">
				<TabsList className="bg-stone-200/50 rounded-none border border-stone-200 p-1 mb-8 w-full md:w-auto flex flex-wrap h-auto">
					<TabsTrigger
						value="create"
						className="rounded-none text-xs uppercase tracking-widest flex-1 md:flex-none"
					>
						Create Model
					</TabsTrigger>
					<TabsTrigger
				...
				<TabsContent value="create" className="focus-visible:ring-0 focus-visible:outline-none">
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
						...
						>
							<FormSection
								title="Preview & Save"
								description="Review the generated image before saving."
							>
						...
						{models.length === 0 ? (
							...
							<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
								{models.map((model) => (
									<div
										key={model.id}
										className="border border-stone-200 bg-white group flex flex-col h-full"
									>
										...
										<div className="p-4 flex-1 flex flex-col">
											...
											<div className="mt-2 space-y-1">
												<div className="flex flex-wrap gap-1">
													{model.ageRange && (
														<span className="text-[9px] bg-stone-100 text-stone-600 px-1.5 py-0.5 uppercase tracking-widest">
															{model.ageRange}
														</span>
													)}
													{model.gender && (
													...
													{model.ethnicity && (
														<span className="text-[9px] bg-stone-100 text-stone-600 px-1.5 py-0.5 uppercase tracking-widest truncate max-w-[80px]">
															{model.ethnicity}
														</span>
													)}
												</div>
